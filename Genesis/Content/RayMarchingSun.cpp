...
RayMarchingSun::~RayMarchingSun()
{
}

void RayMarchingSun::CreateDeviceDependentResources()
{
	auto loadVSTask = DX::ReadDataAsync(L"RayMarchingVS.cso");
	auto loadPSTask = DX::ReadDataAsync(L"RayMarchingSunPS.cso");
	
	auto loadVSQuadTask = DX::ReadDataAsync(L"SunVS.cso");
	auto loadGSQuadTask = DX::ReadDataAsync(L"SunGS.cso");
	auto loadPSQuadTask = DX::ReadDataAsync(L"SunPS.cso");
	...
	auto createSunTask = (createVSTask && createPSTask && createVSQuadTask && createGSQuadTask && createPSQuadTask).then([this]() {
		...
		DX::ThrowIfFailed(
			m_deviceResources->GetD3DDevice()->CreateShaderResourceView(texture.Get(), &srvDesc, &m_sunTextureSRV)
		);
	...
	createSunTask.then([this]() {
		m_ready = true;
	});

}

void RayMarchingSun::ReleaseDeviceDependentResources()
{
	m_ready = false;
...
void RayMarchingSun::Update(DX::StepTimer const& timer, ModelViewProjCB& mvp, XMVECTOR& camPos)
{
...
void RayMarchingSun::Render()
{
	...
	context->PSSetShaderResources(0, 1, &m_sunTextureSRV);
